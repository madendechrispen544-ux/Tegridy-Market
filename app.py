...
<section class="hero">
    ...
    <p>Discover family-plan spots and share eligible digital subscriptions responsibly with people you trust.</p>
    <p class="muted small">TEGRIDY does not provide passwords or bypass platform rules. Always use a service's official family-sharing features and follow its terms.</p>
    {% if current_user.is_authenticated %}
    <a class="btn" href="#post-form">Explore Available Spots</a>
    {% else %}
    <a class="btn" href="#auth">Join the Community</a>
    {% endif %}
</section>

<section class="grid">
    <div class="card">
        <h2>① Find an eligible spot</h2>
        <p class="muted">Browse available family-plan spots by category and read the listing details.</p>
    </div>
    <div class="card">
        <h2>② Check the rules</h2>
        <p class="muted">Confirm that the plan and your intended sharing arrangement are permitted by the platform.</p>
    </div>
    <div class="card">
        <h2>③ Connect safely</h2>
        <p class="muted">Use official invitations or approved account-management features where available. Never post passwords publicly.</p>
    </div>
</section>
...
<section class="card">
    ...
    <div class="grid">
        <div><span class="badge">🎬 Streaming</span><p class="muted small">Movies, series and entertainment family plans.</p></div>
        <div><span class="badge">🎵 Music</span><p class="muted small">Music subscriptions with eligible family options.</p></div>
        <div><span class="badge">🎮 Gaming</span><p class="muted small">Gaming memberships and family features where permitted.</p></div>
        <div><span class="badge">💻 Software</span><p class="muted small">Software plans with legitimate multi-user or family access.</p></div>
    </div>
</section>
...
<section class="card">
    ...
    <ul class="muted">
        <li>Only post spots you are authorized to manage or share.</li>
        <li>Follow the official terms and family-plan rules of every platform.</li>
        <li>Never publish passwords, payment card details, one-time codes or other sensitive credentials.</li>
        <li>Use the report button for misleading or suspicious listings.</li>
    </ul>
</section>
...
<div class="card" id="post-form">
    ...
    <form method="post" action="{{ url_for('create_post') }}">
        <div class="grid">
            <input name="service" maxlength="100" placeholder="Platform or service name" required>
        ...
        <textarea name="description" maxlength="300" placeholder="Describe the eligible family-plan spot and how access is managed without posting passwords..."></textarea>
...
<div class="card">
    ...
    <p><b>Contact / invitation method:</b> {{ post.contact }}</p>
...
<footer class="container" style="padding:30px 0 45px;text-align:center;color:var(--muted);font-size:.9rem">
    <div class="card">
        <b>TEGRIDY FAMILY</b><br>
        A community marketplace for responsible sharing of eligible digital family-plan spots.<br>
        Always follow the official rules and terms of the service you use.
    </div>
</footer>
</body>
</html>
...
def home():
    ...
    category_counts = {item: Post.query.filter_by(category=item).count() for item in categories}
...
def ensure_developer_account():
    ...
    if admin is None:
        admin = User(
            ...
            password=generate_password_hash(os.environ.get("ADMIN_PASSWORD", "change-this-admin-password"))
        )
